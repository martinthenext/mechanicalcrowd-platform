...
def worker(context):
    ...
    while True:
        ...
        print "Worker '%s' receive from client '%s' payload: %s" (ident, client, payload)
    ...
    def backend_routine(socks):
        if backend in socks and socks[backend] == zmq.POLLIN:
            ...
            if ident == event:
                ...
                if payload:
                    print "sending payload to client '%s': %s" % (client, payload)
                    frontend.send_multipart(client, "", event["payload"])
                else:
        ...
        if frontend in socks and socks[frontend] == zmq.POLLIN:
            ...
            if ident == payload and ident.startswith("async"):
                ...
                if frontends.get(client):
                    ...
                    print "sending to worker '%s' payload: %s" % (available, event)
                    backend.send_multipart(available, "", event)
                    workers[available] = False
                    frontends[client] = False
                else:
                    print "could not send payload to worker: %s" % payload
                frontend.send_multipart(ident, "", "OK") # fast answer
